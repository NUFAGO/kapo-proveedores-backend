...
export interface ISolicitudPagoRepository {
  // CRUD básico
  create(data: Omit<SolicitudPago, 'id' | 'createdAt' | 'updatedAt'>): Promise<SolicitudPago>;
  findById(id: string): Promise<SolicitudPago | null>;
  update(id: string, data: Partial<SolicitudPago>): Promise<SolicitudPago | null>;
  delete(id: string): Promise<boolean>;

  // Consultas específicas
  findByExpedienteId(expedienteId: string): Promise<SolicitudPago[]>;
  findByTipoPagoOC(tipoPagoOCId: string): Promise<SolicitudPago[]>;
  ...
  sumMontoSolicitadoByExpedienteAndEstado(expedienteId: string, estado: SolicitudPago['estado']): Promise<number>;
